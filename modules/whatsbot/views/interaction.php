                ...
                <div class="tw-w-full tw-max-w-md">
                    <div class="tw-bg-white tw-shadow-md tw-rounded-lg tw-p-6">
                        ...
                        <div class="tw-text-center">
                            ...
                            <a href="<?= admin_url('settings?group=pusher') ?>" class="tw-bg-blue-500 tw-py-2 tw-px-4 tw-rounded tw-w-full tw-block tw-mb-4"> CRM Settings → Pusher.com</a>
                            <p class="tw-mb-4">For guidance, you can follow this tutorial:</p>
                            <a href="#" target="_blank" class="tw-text-blue-500">See how to set up Pusher from CRM documentation</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    new Vue({
        ...
        methods: {
            ...
            handleBadgeDisplay() {
                ...
                filteredInteractions.forEach(interaction => {
                    ...
                    if (interaction.type === "contacts") {
                        ...
                        // let matchingSource = this.wb_requireData.lead_sources.find(source => source.id === interaction.source);
                        ...
                        // let matchingStatus = this.wb_requireData.lead_status.find(status => status.id === interaction.status);
            ...
            wb_countUnreadMessages(interactionId) {
                const interaction = this.interaction ? this.interactions.find(inter => inter.id === interactionId) : undefined;
                if (interaction) {
                ...
                if (this.wb_searchText) {
                    ...
                    this.wb_displayedInteractions = this.wb_filteredInteractions.filter(interaction => {
                        return Object.keys(interaction).some(key =>
                            interaction[key].toString().toLowerCase().includes(searchText)
                        );
                    });
                } else {
